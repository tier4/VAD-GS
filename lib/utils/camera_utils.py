...
class Camera(nn.Module):
    def __init__(
        ...
        self._image_device = 'cpu'
        if image is not None:
            self._original_image = image.clamp(0, 1)
            self.image_height, self.image_width = self._original_image.shape[1], self._original_image.shape[2]
        else:
...
def _load_guidance_from_path(k, path):
...
def _convert_guidance_to_tensor(k, raw_data, resolution):
...
class LazyGuidanceDict:
    ...
    def __init__(self, sources, resolution):
        self._sources = dict(sources)   # key -> file path (str) or PIL Image
        self._resolution = resolution
        self._cache = {}                # key -> loaded tensor / value
        self._persistent = set()        # keys set via __setitem__
    ...
    def _load(self, key):
        ...
        if isinstance(source, str):
            raw = _load_guidance_from_path(key, source)
        else:
            raw = source
        if raw is None:
            return None
...
def loadCam(cam_info: CameraInfo, resolution_scale, scale=1.0):
    ...
    guidance = LazyGuidanceDict(cam_info.guidance, resolution)
