...
cfg.train.bg_only_finetune = False  # If True, freeze all actor + actor_pose params, render main composite without actors (exclude_list=obj_list), and mask dynamic-mask pixels out of the L1/SSIM/PSNR. Used for the segmented-merge Phase 3 to polish BG only — sidesteps the opt_pose mismatch that corrupts actors loaded via obj_init_from. Sky still trains normally.
cfg.train.bg_actor_bbox_prune_enable = False  # If True, after bg_init_from, drop BG Gaussians whose center sits inside any actor's 3D bbox at any frame. Catches "ghost actor" BG Gaussians that merge step concatenates from per-segment training (where actor opacity was low early on and L1 leaked actor color into BG). Orthogonal to bg_lidar_prune (which misses BG inside actor bodies because LiDAR returns match actor surface = not a conflict).
cfg.train.bg_actor_bbox_prune_scale_mul = 1.0  # Pad each Gaussian's OBB inclusion test by scale_mul * max(scaling). 1.0 = "1-sigma ellipsoid touches bbox" criterion. Lower (0.5) = stricter (only deep overlap pruned); higher (2.0) = looser (anything within 2-sigma counted).
cfg.train.bg_actor_bbox_prune_pad = 0.05  # Absolute extra padding (m) on top of scale-based pad. 0.05m catches Gaussians whose ellipsoid is small but center is barely outside the bbox.
