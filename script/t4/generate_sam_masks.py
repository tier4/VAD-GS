"""Generate dynamic instance masks (sam_masks) for T4 datasets.

SAM3 text prompts per category ("car", "truck", "bus", "person",
"bicycle", "motorcycle") detect all instances, then greedy IoU matching
assigns each SAM3 mask to a projected 3D bounding box.

Background masks are generated separately by generate_bkgd_masks.py.

Usage:
    python script/t4/generate_sam_masks.py --config configs/example/t4_train_example.yaml
    python script/t4/generate_sam_masks.py --dataroot caf37e66-... --scene-index 0
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate dynamic instance masks for T4 dataset"
    )
    ...
    out_dynamic = args.output_dir or (dataroot / "preprocessed" / "sam_masks")
    ...
    for frame in ds.iter_frames(args.camera_channels):
        ...
        if args.skip_existing and dyn_path.exists():
            continue
    ...
    print(f"Done. Dynamic masks: {out_dynamic}")
